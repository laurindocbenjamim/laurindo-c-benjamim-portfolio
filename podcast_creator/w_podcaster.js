        ...
        // Removed audioControls as it's no longer a separate container for buttons
        ...
        const recordedAudioDownloadFormatSelect = document.getElementById('recorded-audio-download-format'); // New select for download format
        ...
        toggleLoopButton.style.backgroundColor = '#cf3333'; // Set initial background color for loop button
        toggleLoopButton.style.color = '#ffffff'; // Set initial text color for loop button
        const loopOffIcon = document.getElementById('loop-off-icon');
        const loopOnIcon = document.getElementById('loop-on-icon');
        ...
        const attachAudioListeners = () => {
            ...
            if (activeAudioElement) {
                ...
                activeAudioElement.onended = () => {
                    // If loop is on, it will automatically restart, so no need to change isPlaying state
        ...
        const resetVisualizationAndControls = () => {
            ...
            loopOffIcon.style.display = 'inline';
        };
        ...
        const loadRecordedAudio = (blob) => {
            ...
            // Clean up previous dynamically created recorded audio element if it's exists
            if (currentRecordedAudioElement) {
                currentRecordedAudioElement.pause();
            ...
            // Update the general download link for the active audio
            // The actual download format will be determined by the backend based on user selection
            downloadActiveAudioLink.href = currentRecordedAudioElement.src; 
            downloadActiveAudioLink.download = `recorded_audio.${recordedAudioDownloadFormatSelect.value}`; // Use selected format for filename
            downloadActiveAudioLink.style.display = 'inline-flex';
            recordedAudioDownloadFormatSelect.style.display = 'inline-block'; // Show for recorded audio
            messageElement.textContent = ''; // Clear previous warning
        ...
        toggleLoopButton.addEventListener('click', () => {
            if (!activeAudioElement) {
                showErrorAlert('No audio loaded to loop.');
                return;
            }
            activeAudioElement.loop = !activeAudioElement.loop;
            if (activeAudioElement.loop) {
                toggleLoopButton.style.backgroundColor = '#61fd2d'; // Change background color when loop is on
                toggleLoopButton.style.color = '#000000'; // Change text color when loop is on
                loopOnIcon.style.display = 'inline';
                loopOffIcon.style.display = 'none';
            } else {
                loopOnIcon.style.display = 'none';
                loopOffIcon.style.display = 'inline';
                toggleLoopButton.style.backgroundColor = '#cf3333'; // Reset background color when loop is off
                toggleLoopButton.style.color = '#ffffff'; // Reset text color when loop is off
            }
        });
        ...
        recordedAudioDownloadFormatSelect.addEventListener('change', (event) => {
            // Update the download link's filename based on the selected format
            if (activeAudioElement && activeAudioElement === currentRecordedAudioElement) {
                downloadActiveAudioLink.download = `recorded_audio.${event.target.value}`;
            }
            // No warning message needed here as backend handles conversion
            messageElement.textContent = ''; 
        });
        ...
        const generatePodcastDescription = () => {
        ...
        generateDescriptionButton.addEventListener('click', generatePodcastDescription);
        generateHashtagsButton.addEventListener('click', generateSuggestedHashtags);


        generateVideoButton.addEventListener('click', async () => {
            // Check if at least one audio source is available
            if (!uploadedAudioFileBlob && !recordedAudioFileBlob) {
            ...
            // Append uploaded audio if it exists
            if (uploadedAudioFileBlob) {
                formData.append('uploadedAudio', uploadedAudioFileBlob, uploadedAudioFileBlob.name || 'uploaded_audio.webm');
            }

            // Append recorded audio if it exists
            if (recordedAudioFileBlob) {
                formData.append('recordedAudio', recordedAudioFileBlob, 'recorded_audio.webm'); // Use a generic name for recorded audio
            }
            ...
            // Append download format for recorded audio
            // This now dictates the audio encoding format within the final MP4 video
            // If both are sent, the backend will need logic to decide which audio to use or how to combine them.
            // For now, we'll send the format based on which was active, but the backend will decide.
            if (activeAudioElement === currentRecordedAudioElement) {
                formData.append('downloadFormat', recordedAudioDownloadFormatSelect.value);
            } else {
                formData.append('downloadFormat', 'mp3'); 
            }
            ...
            try {
                ...
                // Use the backend URL for the API call
                const response = await fetch('https://192.168.1.252:8443/api/v2/podcast/generate', { // Use http://127.0.0.1:5000 for local Flask
                    method: 'POST',
                    body: formData,
                ...
                if (result && result.video_url) {
                    // Update download link to point to the backend's download endpoint
                    downloadLink.href = `https://192.168.1.252:8443${result.video_url}`;
                    // The backend always outputs MP4 video, so set download filename accordingly
