document.addEventListener('DOMContentLoaded', function () {
    ...
    function saveRecording() {
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                <input type="checkbox" class="form-check-input track-checkbox me-2" id="merge-${trackId}">
                <div class="track-title" data-id="${trackId}">
    ...
    `;
        ...
        const mergeCheckbox = trackElement.querySelector('.track-checkbox');
        ...
        // Track title click for renaming
        trackTitle.addEventListener('click', () => {
            currentRenamingTrack = trackId;
            document.getElementById('trackNameInput').value = trackTitle.textContent.trim();
            const renameModal = new bootstrap.Modal(document.getElementById('renameTrackModal'));
            renameModal.show();
        });
        ...
        // Add merge checkbox event
        mergeCheckbox.addEventListener('change', updateMergeButton);
    }

    // Add this function for the edit modal
    ...
    function updateMergeButton() {
        ...
        if (!mergeBtn) {
            // Create merge button if it doesn't exist
            ...
            <button class="btn btn-success merge-btn" id="mergeBtn">
                <i class="fas fa-random me-1"></i> Merge Selected Tracks with Video
            </button>
        `;
            tracksContainer.appendChild(mergeSection);

            document.getElementById('mergeBtn').addEventListener('click', mergeTracksWithVideo);
        }

        document.getElementById('mergeBtn').disabled = checkedBoxes.length === 0;
    }

    function mergeTracksWithVideo() {
        const checkedBoxes = document.querySelectorAll('.track-checkbox:checked');
        const replaceOriginal = document.getElementById('replaceOriginal').checked;
        const trackIds = Array.from(checkedBoxes).map(cb => cb.id.replace('merge-', ''));

        // In a real app, this would actually merge the tracks with the video
        alert(`Would merge tracks: ${trackIds.join(', ')}\nReplace original: ${replaceOriginal}`);

        // Simulate processing
        const mergeBtn = document.getElementById('mergeBtn');
        const originalText = mergeBtn.innerHTML;
        mergeBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i> Merging...';
        mergeBtn.disabled = true;

        setTimeout(() => {
            mergeBtn.innerHTML = originalText;
            mergeBtn.disabled = false;
            alert('Merge completed! (simulated)');
        }, 2000);
    }

    // Add event listener for the rename modal save button
    document.getElementById('saveTrackNameBtn').addEventListener('click', () => {
        if (currentRenamingTrack) {
            const newName = document.getElementById('trackNameInput').value.trim();
            if (newName) {
                const trackTitle = document.querySelector(`.track-title[data-id="${currentRenamingTrack}"]`);
                if (trackTitle) {
                    trackTitle.innerHTML = `<strong>${newName}</strong>${trackTitle.innerHTML.match(/<span class="text-muted ms-2">.*<\/span>/)[0]}`;
                }
            }
